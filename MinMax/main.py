...
data = data.split(' ')
inputText.close
first = str(raw_input("Input first word: "))
second = str(raw_input("Input second word: "))
#определяем все вхождения первого и второго слова в исходном тексте
firstInputs = [i for i in xrange(len(data)) if data[i] == first]
secondInputs = [i for i in xrange(len(data)) if data[i] == second]
...
for i in firstInputs:
    for j in secondInputs:
        maxMin = abs(i - j)
#если вычесленное расстояние больше минимального и второй индекс больше первого, то переходим к следующему вхождению первого слова
